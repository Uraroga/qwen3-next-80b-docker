...
mostra_uso() {
  cat <<'EOF'
Uso:
  ./scripts/build-atlas5.sh [opzioni]

Costruisce e verifica l'immagine Docker llama-server destinata al nodo atlas5.
...
Note:
  ...
  - Lo script non avvia llama-server e non pubblica porte di rete.
EOF
}
...
carica_configurazione() {
  ...
  if [[ "$MAIN_CPU_PROFILE" != "avx2" ]]; then
    errore "MAIN_CPU_PROFILE deve essere obbligatoriamente 'avx2', rilevato: $MAIN_CPU_PROFILE"
    exit 1
  fi
}
...
mostra_riepilogo() {
  ...
  info
  info "Riepilogo build llama-server atlas5"
  info "Hostname                  : $hostname_corrente"
  info "Dockerfile utilizzato     : $repo_root/atlas5/Dockerfile.server"
  info "LLAMA_CPP_REF             : $LLAMA_CPP_REF"
  info "LLAMA_SERVER_IMAGE        : $LLAMA_SERVER_IMAGE"
  info "Profilo CPU richiesto     : $MAIN_CPU_PROFILE"
...
chiedi_conferma() {
  ...
  local risposta=""
  printf 'Continuare con la build dell'\''immagine llama-server per atlas5? [s/N] '
...
verifica_immagine() {
  ...
  "${docker_cmd[@]}" image inspect "$LLAMA_SERVER_IMAGE" >/dev/null

  info "Immagine costruita:"
  "${docker_cmd[@]}" image ls "$LLAMA_SERVER_IMAGE"
}
...
verifica_container_help() {
  ...
  if ! "${docker_cmd[@]}" run --rm "$LLAMA_SERVER_IMAGE" --help >"$output_test" 2>&1; then
    cat "$output_test"
...
info
info "Build e verifica dell'immagine llama-server completate correttamente."
